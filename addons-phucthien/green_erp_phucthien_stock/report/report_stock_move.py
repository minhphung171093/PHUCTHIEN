...
class report_stock_inventory(osv.osv):
    ...
    _columns = {
        'tracking_id': fields.many2one('stock.tracking', 'Kiện', readonly=True),
    }
    def init(self, cr):
...
CREATE OR REPLACE view report_stock_inventory AS (
    (SELECT
        ...
        m.company_id,
        m.state as state, m.tracking_id as tracking_id, m.prodlot_id as prodlot_id,
    ...
    GROUP BY
        m.id, m.product_id, m.product_uom, pt.categ_id, m.partner_id, m.location_id,  m.location_dest_id, m.tracking_id,
