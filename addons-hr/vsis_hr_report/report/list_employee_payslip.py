...
class Parser(report_sxw.rml_parse):
    ...
    def get_data(self,data):
        ...
        sql2 = '''  select emp.id as employee_id, emp.name_related as emp_name, emp.last_name as last,
  ...
  '''%(data['date_start'],data['date_stop'],data['date_start'],data['date_stop'],tuple(data['department']))
        ...
        for line in data_emp:
            ...
            if not flag_department:
                ...
                data_sum_rule.update(line_rule)
            self.stt += 1
            name =  line['last'] and line['last'] + ' ' + line['emp_name'] or line['emp_name']
    ...
    def get_rule(self,form):
        rule = []
        sql1 = '''  select distinct(rule_cate.id),rule_cate.name,temp_line.sequence 
                from gs_hr_template temp 
                left join gs_hr_template_line temp_line on temp_line.template_id = temp.id 
                left join hr_salary_rule_category rule_cate on rule_cate.id = temp_line.name
                where temp.id = %s
                order by temp_line.sequence asc''' %(form['template'][0])
        self.cr.execute(sql1)
        for line in self.cr.dictfetchall():
            dic = {
                   'id_rule': line['id'],
    ...
    def get_department(self,form):
        result = []
        if form and len(form['department'])<=1:
            form['department'].append(-1)
        ...
        where sl.date_from >= '%s' 
            ...
            and dep.id in %s
        order by dep.name 
  '''%(form['date_start'],form['date_stop'],form['date_start'],form['date_stop'],tuple(form['department']))
        self.cr.execute(sql2)     
        for l in self.cr.dictfetchall():
            dic = {
    ...
    def get_emp(self,form,id_department):
        stt = 0
        result = []
        sql3 = '''  select emp.id as employee_id, emp.name_related as emp_name, emp.last_name as last,sl.id as id_slip, bak.acc_number as numberbank_emp, bak.bank_name as bank_name
        from hr_payslip sl
        left join hr_employee emp on emp.id = sl.employee_id
        left join hr_department dep on dep.id = emp.department_id
        ...
        where sl.date_from >= '%s' 
            ...
            and sl.date_to <='%s'
            and dep.id = %s
  '''%(form['date_start'],form['date_stop'],form['date_start'],form['date_stop'],id_department)
        self.cr.execute(sql3)
        for line in self.cr.dictfetchall():
            stt += 1
            dic = {
                   ...
                   'name_emp': line['last'] and line['last'] + ' ' + line['emp_name'] or line['emp_name'] ,
                   'id_slip': line['id_slip'],
                   'numberbank_emp': line['numberbank_emp'],
                   'bank_name': line['bank_name'],
                   'name_emp_bank': line['last'] and func_bodau.bo_dau_viet(line['last']).upper() + ' ' + func_bodau.bo_dau_viet(line['emp_name']).upper() or func_bodau.bo_dau_viet(line['emp_name']).upper() ,
            }
            result.append(dic)
        return result
    
    
    def get_amount(self,form,id_department,id_slip,id_rule):
        amount = 0
        sql4 = '''  select sum(line.amount) as amount 
            from hr_payslip sl
            left join hr_payslip_line line on sl.id = line.slip_id
            left join hr_salary_rule rule on rule.id = line.salary_rule_id
            left join hr_salary_rule_category rule_cate on rule_cate.id = rule.category_id
            left join hr_employee emp on emp.id = sl.employee_id
            left join hr_department dep on dep.id = emp.department_id
            where sl.date_from >= '%s' 
            and sl.date_from <= '%s'
            ...
            and sl.id = %s
            and rule_cate.id = %s 
            group by rule_cate.id
      '''%(form['date_start'],form['date_stop'],form['date_start'],form['date_stop'],id_department,id_slip,id_rule)
#        print 'sql4',sql4 
    ...
    def get_total_work(self,form,id_department):
    ...
    def get_total_business(self,form,id_department):
        list_emp = self.get_emp(form, id_department)
        total = 0
        if list_emp:
            for emp in list_emp:
    ...
    def get_total_dep(self,form,id_department,id_rule):
        total = 0
        list_emp = self.get_emp(form, id_department)
        for emp in list_emp:
            temp = self.get_amount(form, id_department, emp['id_slip'], id_rule)
            if temp:
                total += temp
        return total
            
    def get_total_all_dep(self,form,id_rule):
        total = 0
        list_dep = self.get_department(form)
        for d in list_dep:
            temp = self.get_total_dep(form,d['id_department'], id_rule)
            if temp:
                total += temp
        return total
