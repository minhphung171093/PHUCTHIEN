# Embedded file name: D:\Working\Version_7.0\OpenERPV7\addons_hr\vsis_hr_attendance\wizard\vsis_hr_attendance_wizard.py
import netsvc
import time
import pooler
...
class vsis_hr_attendance_wizard(osv.osv_memory):
    ...
    _columns = {'date_from': fields.date('Date From'),
                ...
                for day in sorted(list_day_emp):
                    ...
                    if not list_signin or not list_signout:
                        ...
                        if count_signin != count_signout:
                            ...
                            if overtime_ids:
                                overtime = hr_overtime_obj.browse(cr, uid, overtime_ids[0], context=context).number_of_hours
                            val = {'name': 'approve',
                             'employee_id': employee_id,
                             'date': date1,
...
class hr_attendance_report(osv.osv_memory):
    ...
    _columns = {'month': fields.integer('Month', required=True),
    ...
    def print_report(self, cr, uid, ids, context = None):
        ...
        return {'type': 'ir.actions.report.xml',
         'report_name': 'hr_attendance_report',
         'datas': datas}
